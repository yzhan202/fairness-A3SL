 ...
 */
public class EqualOddsMPEInference_compas implements ModelApplication {
	
	private static final Logger log = LoggerFactory.getLogger(EqualOddsMPEInference_compas.class);
	...
	public EqualOddsMPEInference_compas(Model model, Database db, ConfigBundle config,
			Set<String> protectedGroup, Map<String, Integer> truthMap,
			int posProtectedNum, int posUnprotectedNum, int negProtectedNum, int negUnprotectedNum, double sigma) 
		...
		this.protectedNum = new int[] {negProtectedNum, posProtectedNum};
		this.unprotectedNum = new int[] {negUnprotectedNum, posUnprotectedNum};
...
//		log.info("Grounding out model.");
		 ...
		 */
		StandardPredicate positive = (StandardPredicate)PredicateFactory.getFactory().getPredicate("recid");
		for (int i=0; i<=1; i++) {
			/*
			...
			for (GroundAtom atom : Queries.getAllAtoms(db, positive)) {
				GroundTerm[] terms = atom.getArguments();
				// check if a student affiliates to high-rank institute
				String user = terms[0].toString();
				if (truthMap.get(user) == i) {
					atomList.add(atom);
					if (!protectedGroup.contains(user)) { // Unprotected Group
