 ...
 */
public class compasMPEInference implements ModelApplication {
	
	private static final Logger log = LoggerFactory.getLogger(compasMPEInference.class);
	...
	public compasMPEInference(Model model, Database db, ConfigBundle config,
			Set<String> protectedGroup, int protectedNum, int unprotectedNum, double sigma) 
...
//		log.info("Grounding out model.");
		...
		for (GroundAtom atom : Queries.getAllAtoms(db, positive)) {
			GroundTerm[] terms = atom.getArguments();
			// check if a student affiliates to high-rank institute
			String authorName = terms[0].toString();
			atomList.add(atom);
